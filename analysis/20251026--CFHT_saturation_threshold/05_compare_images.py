...
#
# Compare the raw and processed images. Try to figure out the saturation
# threshold that the pipeline uses. I suspect something around 45k.
#
# Rob Siverd
# Created:       2025-01-26
...
##--------------------------------------------------------------------------##
## Load images:
ofile = 'example1/ne_2413738o.fits.fz'
pfile = 'example1/ne_2413738p.fits.fz'
odata, ohdrs = pf.getdata(ofile, header=True)
pdata, phdrs = pf.getdata(pfile, header=True)

idiff = odata - pdata

##--------------------------------------------------------------------------##
## DS9 hint:
sys.stderr.write("\nFollow along with this command:\n")
sys.stderr.write("flztf %s %s\n" % (ofile, pfile))
sys.stderr.write("flztf example1/*.fits.fz\n")

## Select zeroed-out pixels:
baddies = (pdata == 0)

## What *were* they before:
asdf = odata[baddies]

## What are the highest original pixel values that are NOT zeroed?
kept = odata[~baddies]
...
######################################################################
# CHANGELOG (05_compare_images.py):
#---------------------------------------------------------------------
#
#  2025-01-26:
#     -- Increased __version__ to 0.0.1.
#     -- First created 05_compare_images.py.
#
