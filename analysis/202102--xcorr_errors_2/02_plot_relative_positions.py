...
#
# Plot relative shifts of images based on header information. I will compare
# this to what the cross-correlation is finding to hopefully identify the
# source of the ongoing problem.
#
# Rob Siverd
# Created:       2021-02-25
...
##--------------------------------------------------------------------------##
## Some Spitzer info:
arcsec_per_pixel = 1.2232
pixels_per_deg   = 3600.0 / arcsec_per_pixel

##--------------------------------------------------------------------------##
## Quick ASCII I/O:
data_file = 'header_wcs_posn.txt'
#data_file = 'data.txt'
gftkw = {'encoding':None} if (_have_np_vers >= 1.14) else {}
gftkw.update({'names':True, 'autostrip':True})
#gftkw.update({'delimiter':'|', 'comments':'%0%0%0%0'})
#gftkw.update({'loose':True, 'invalid_raise':False})
hdr_info = np.genfromtxt(data_file, dtype=None, **gftkw)
#all_data = aia.read(data_file)
dra, dde = hdr_info['CRVAL1'], hdr_info['CRVAL2']


## Approximate average position:
avg_ra = np.average(dra)
avg_de = np.average(dde)
cosdec = np.cos(np.radians(avg_de))
rel_ra = pixels_per_deg * (dra - avg_ra) * cosdec
rel_de = pixels_per_deg * (dde - avg_de)
cczero = 1
rel_ra -= rel_ra[cczero]
rel_de -= rel_de[cczero]

##--------------------------------------------------------------------------##
## Retry using real WCS:
ipaths = hdr_info['FILENAME']
ibases = [os.path.basename(x) for x in ipaths]
imhdrs = [pf.getheader(x) for x in ipaths]
imgwcs = [awcs.WCS(x) for x in imhdrs]

## Pick a coordinate that contained by all images:
ref_ra = dra[0]
ref_de = dde[0]

## Convert reference coordinate to pixel position in each case:
#xpixels, ypixels = [], []
pixpos = [ww.all_world2pix(ref_ra, ref_de, 1) for ww in imgwcs]
xx, yy = np.array(pixpos).T
...
######################################################################
# CHANGELOG (02_plot_relative_positions.py):
#---------------------------------------------------------------------
#
#  2021-02-25:
#     -- Increased __version__ to 0.0.1.
#     -- First created 02_plot_relative_positions.py.
#
