...
##--------------------------------------------------------------------------##
## Configuration:
irac_n_pixels = 256. * 256.     # standard IRAC frame is 256x256
maic_n_pixels = 512. * 512.     # padded IRAC frame (stack) dimensions
root_data_dir = '/home/rsiverd/ucd_project/ucd_targets/'

stack_files = sfh.get_files_walk(root_data_dir, 'clean_stack')
medze_files = sfh.get_files_walk(root_data_dir, 'clean_medze')
sys.stderr.write("Found %d stack files.\n" % len(stack_files))
sys.stderr.write("Found %d medze files.\n" % len(medze_files))

stack_tags = [sfh.get_irac_aor_tag(x) for x in stack_files]
medze_tags = [sfh.get_irac_aor_tag(x) for x in medze_files]
uniq_tags = list(set(stack_tags + medze_tags))
sys.stderr.write("Found %d unique tags.\n" % len(uniq_tags))

## Organize paths by tag:
stack_fdict = {tt:pp for tt,pp in zip(stack_tags, stack_files)}
medze_fdict = {tt:pp for tt,pp in zip(medze_tags, medze_files)}

##--------------------------------------------------------------------------##
## NaN-counter routine:
def nancount(idata):
    return np.sum(np.isnan(idata))
...
for tag in uniq_tags:
    ...
    sfile = stack_fdict.get(tag)
    mfile = medze_fdict.get(tag)
    if not sfile:
        sys.stderr.write("Error: no stack file for tag %s!\n" % tag)
        continue
    if not mfile:
        sys.stderr.write("Error: no medze file for tag %s!\n" % tag)
        continue

    # load pixel data:
    sdata = fitsio.read(sfile)
    mdata = fitsio.read(mfile)

    # count NaNs:
    s_nans.append(nancount(sdata))
    m_nans.append(nancount(mdata))

## Promote to numpy arrays, compute good pixel count:
s_nans = np.array(s_nans)
m_nans = np.array(m_nans)


s_kept = maic_n_pixels - s_nans
m_kept = maic_n_pixels - m_nans
s_frac = s_kept / irac_n_pixels
m_frac = m_kept / irac_n_pixels
...
hbins = 50
epsilon = 2.0
hopts = {'bins':hbins, 'range':(0+epsilon, 200+epsilon)}
ax1.hist(s_pctg, **hopts)
ax2.hist(m_pctg, **hopts)
...
ax1.set_ylabel("Number of stacks (minimal)")
ax2.set_ylabel("Number of stacks (maximal)")

ax2.set_xlabel("Pixels Preserved (%)")
...
######################################################################
# CHANGELOG (05_compare_stack_methods.py):
#---------------------------------------------------------------------
#
#  2021-02-08:
#     -- Increased __version__ to 0.0.1.
#     -- First created 05_compare_stack_methods.py.
#
