...
def squared_residuals_foc2ccd_rdist(params, diags=False):
    ...
    for qq,gst in gstars.items():
        ...
        #test_xrel, test_yrel = helpers.inverse_tan_cdmcrv(cdmcrv,
        #                            gstars[qq]['gra'], gstars[qq]['gde'])
        test_xrel, test_yrel = helpers.inverse_tan_cdmcrv_distmod(cdmcrv,
                           test_distmod, gstars[qq]['gra'], gstars[qq]['gde'])
                    
        if diags:
            nodist_xrel, nodist_yrel = helpers.inverse_tan_cdmcrv(cdmcrv,
                                        gstars[qq]['gra'], gstars[qq]['gde'])
            xnudge = test_xrel - nodist_xrel
            ynudge = test_yrel - nodist_yrel
