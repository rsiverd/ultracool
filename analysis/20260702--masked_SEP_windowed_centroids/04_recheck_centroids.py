...
#
# Simple centroid check on the PSF stamps I produced.
#
# Rob Siverd
# Created:       2026-07-07
...
try:
    ...
    sys.stderr.write("Error: sep module not found!\n\n")
    sys.exit(1)
...
## Get dimensions from first:
mm1 = list(psf_data.keys())[0]
im1 = psf_data[mm1]
ny, nx = im1.shape
...
psf_ctrs = {}
for mm,psf in psf_data.items():
    itot = np.sum(psf)
    xpos = np.sum(psf * xx) / itot      # array units (0-indexed)
    ypos = np.sum(psf * yy) / itot      # array units (0-indexed)
    psf_ctrs[mm] = (xpos + 1, ypos + 1)
    pass

## Announce:
sys.stderr.write("\nPSF centroids:\n")
sys.stderr.write("               Xdumb,   Ydumb,    XWIN,    YWIN\n")
for mm,(cx,cy) in psf_ctrs.items():
    se_xwin = cat_data[mm]['XWIN_IMAGE']
    se_ywin = cat_data[mm]['YWIN_IMAGE']
    sys.stderr.write("%s --> %.4f, %.4f, %.4f, %.4f\n"
                     % (mm, cx, cy, se_xwin, se_ywin))
...
######################################################################
# CHANGELOG (04_recheck_centroids.py):
#---------------------------------------------------------------------
#
#  2026-07-07:
#     -- Increased __version__ to 0.0.1.
#     -- First created 04_recheck_centroids.py.
#
