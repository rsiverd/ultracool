...
## Evaluator routine for CD matrix + CRVALs. This variant includes 4 CD
## matrix parameters (CD1_1, CD1_2, CD2_1, CD2_2) which are fixed for all
## images in the AOR plus 2 CRVAL parameters per image. The total number
## of parameters is thus 4 + 2*N, where N is the number of images in the AOR.
def cdmat_evaluator(params, imkeys, matches):
    cdmat = params[:4]
...
## Optionally override the default pixel scales for computation:
akspoly._pxscale[1] = 1.22328
akspoly._pxscale[2] = 1.21451

## Evaluator routine for position angle (PA) + CRVALs. This variant includes 
...
def padeg_evaluator(params, channel, imkeys, matches):
    ...
    for ii,kk in enumerate(imkeys):
        ...
        #sys.stderr.write("Checking %s (%f, %f) ... \n" % (kk, cv1, cv2))
        cxx, cyy, gra, gde = (np.array(x) for x in zip(*matches[kk]))
        cra, cde = akspoly.xypa2radec(padeg, cxx, cyy, cv1, cv2, channel)
        deltas += angle.dAngSep(cra, cde, gra, gde).tolist()
...
## Choose which evaluator gets used:
#evaluator = cdmat_evaluator
...
max_imgs = 30       # skip AORs that have more than this many images
...
## How to save results:
save_cols_cdmat = ['iname', 'aor_tag', 'jdtdb', 'padeg',
...
for aor_tag in unique_tags:
    ...
    if n_tagged < 2:
        ...
        sys.exit(1)
    sys.stderr.write("Found %d files in AOR %s ...\n" % (n_tagged, aor_tag))
    if n_tagged > max_imgs:
        sys.stderr.write("Skipping huge AOR ...\n")
        continue
    #continue
    ...
    # Note which channel we are using:
    aor_channel = int(aor_tag.split('_')[1][1])
    ...
    # special handling for PA:
    #pa_phi = (np.array(list(wstuff['PA'].values())) % 360.0) / 360.0
    #pa_avg = angle.circ_avg_phase(pa_phi)
    pa_phase_all = (np.array(list(wstuff['PA'].values())) / 360.0)
    pa_phase_avg = angle.circ_avg_phase(pa_phase_all)
    pa_deg_avg   = pa_phase_avg * 360.0
    if pa_deg_avg > 180.0:
        pa_deg_avg -= 360.0
    paravg['PA'] = pa_deg_avg


    # build array of initial guesses:
    if fit_kind is 'CD':
        param_guess = [paravg['CD1_1'], paravg['CD1_2'],
                        paravg['CD2_1'], paravg['CD2_2']]
    if fit_kind is 'PA':
        param_guess = [paravg['PA']]
    for kk in imkeys:
    ...
    if fit_kind is 'PA':
        ...
        bestfit_crval = bestfit_pars[1:].reshape(-1, 2)
        guessed_padeg = param_guess[:1]
