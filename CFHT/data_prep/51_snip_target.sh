#!/bin/bash
#
# Snip out desired quadrant for a specific target, specified in config.sh.
# This is a target-agnostic version of 01_snip_calib1.sh.
#
# Rob Siverd
# Created:      2023-05-25
...
## Folders:
[ -z "$data_dir" ] && PauseAbort "Variable data_dir not defined!"
...
#save_dir="./${target}_p_${use_quad}/download"
...
echo "fitsarith: `/usr/bin/which fitsarith`"
...
while read ipath <&10; do
   ...
   # count HDUs:
   echo "ipath: $ipath"
   numHDUs=$(liststruc ${ipath} | grep '^HDU' | wc -l)
   #numHDUs=$(liststruc ${ibase} | grep '^HDU' | wc -l)
   echo "numHDUs: $numHDUs"
   if [[ $numHDUs -lt 4 ]]; then
      echo "Too few HDUs ... skip this one ..."
      continue
   fi

   # snip quadrant:
   #cmde "fitsarith $fopts -i '${ibase}[${use_qext}]' -o '!$baz'"   || exit $?
...
######################################################################
# CHANGELOG (01_snip_target.sh):
#---------------------------------------------------------------------
#
#  2023-10-24:
#     -- Increased script_version to 0.10.
#     -- Renamed script to 51_snip_target.sh.
#
#  2023-05-25:
