...
#
# Plot distributions of scaled RA and Dec residuals.
#
# Rob Siverd
# Created:       2021-12-06
...
if not csv_files:
    sys.stderr.write("Error: no CSV files found in folder: %s\n" % resid_dir)
    sys.exit(1)

## Load data from files:
res_data = pd.concat([pd.read_csv(x) for x in csv_files])

## Breakout:
ra_resids = res_data.scaled_ra_resid
de_resids = res_data.scaled_de_resid
...
#pdkwargs.update({'delim_whitespace':True, sep='|'}
...
##--------------------------------------------------------------------------##
## A Gaussian distribution for overplotting:
def gaussian(x, mu, sigma):
    zz = (x - mu) / sigma
    inv_norm = np.sqrt(2.0 * np.pi) * sigma
    return np.exp(-0.5 * zz * zz) / inv_norm
...
ndata = len(ra_resids)
xvals = np.linspace(-7., 7., 1000)
#ndist = float(ndata) * gaussian(xvals, 0.0, 1.0)
ndist_10 = gaussian(xvals, 0.0, 1.0)
ndist_13 = gaussian(xvals, 0.0, 1.34)
ndist_15 = gaussian(xvals, 0.0, 1.5)
ndist_20 = gaussian(xvals, 0.0, 2.0)

def overplot_gaussians(ax):
    ax.plot(xvals, ndist_10, c='r', lw=2, label=r'$\sigma=1.0$')
    #ax.plot(xvals, ndist_13, c='m', lw=2, label=r'$\sigma=1.34$')
    ax.plot(xvals, ndist_15, c='g', lw=2, label=r'$\sigma=1.5$')
...
ax1.grid(True)
ax1.hist(ra_resids, **hopts)
overplot_gaussians(ax1)
ax1.set_xlabel("Scaled RA Residuals")
ax1.legend(loc='upper right')

ax2 = fig.add_subplot(122)
ax2.grid(True)
ax2.hist(de_resids, **hopts)
overplot_gaussians(ax2)
ax2.set_xlabel("Scaled DE Residuals")
...
#plot_name = 'scaled_residuals.png'
plot_name = 'scaled_%s.png' % resid_dir
...
######################################################################
# CHANGELOG (21_residual_distribution.py):
#---------------------------------------------------------------------
#
#  2021-12-06:
#     -- Increased __version__ to 0.1.0.
#     -- First created 21_residual_distribution.py.
#
