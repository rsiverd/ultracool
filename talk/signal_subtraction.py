...
#
# Illustrate a really nice systematics removal.
#
# Rob Siverd
# Created:       2020-02-13
...
omega = 2*np.pi / npts * 2
truth = np.sin(omega * t)

which = slice(None, None, 7)
error = np.zeros_like(truth)
error[which] += 0.5*np.random.randn(error.size)[which]

clean_noise = 0.05 * np.random.randn(t.size)

clean_truth = truth + clean_noise
dirty_truth = clean_truth + error
...
ax1.grid(True)
ax1.scatter(t, dirty_truth, color='purple')
ax1.set_ylim(-1.5, 1.5)

ax2 = fig.add_subplot(312)
ax2.grid(True)
ax2.scatter(t, error, color='r')
ax2.set_ylim(-1.5, 1.5)

ax3 = fig.add_subplot(313)
ax3.grid(True)
ax3.scatter(t, clean_truth)
...
plot_name = 'error_subtraction.png'
...
######################################################################
# CHANGELOG (signal_subtraction.py):
#---------------------------------------------------------------------
#
#  2020-02-13:
#     -- Increased __version__ to 0.1.0.
#     -- First created signal_subtraction.py.
#
